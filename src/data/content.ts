...
export const downloadForm = {
  ...
  /*
   * **ここは配布しているPDFの目次であって、訴求文ではない。**
   * 実物に無いものを書かない・実物にある強いものを落とさない、の2点だけで決める
   * （2026-08-29 全25ページを突合して改訂）。
   *
   * 改訂の理由:
   *   - 「導入企業の声」→ 削除。**PDFに1件も無かった**（全文検索で0件）。
   *   - 「業務効率化の具体事例」→ 実体はp13の**業界平均の目安表**で、顧客名も業種の文脈も無い。
   *     「事例」と書くと、受け取った人が期待したものと違うものを開くことになるので、
   *     中身のとおり「目安」と書く。
   *   - 「4本柱の支援内容詳細」→ p08は各1〜2行なので「詳細」を外す。
   *   - AI文例集（p14-18・5ページ）と助成金（p22）は**実在するのに告知していなかった**。
   *     とくに文例集は資料内で最大のブロックで、そのままコピーして使える形になっている。
   *
   * 助成金に**割合の数字を書いていないのは意図的**。受給条件があるものをLP側で
   * 「最大◯%」と出すと、その場では確かめようのない約束に読める。数字と条件は資料の中にある。
   */
  materials: [
    "AI実装研修の全体像",
    "4本柱の支援内容",
