...
const hr: Industry = {
  ...
  hero: {
    /*
     * 2026-08-11 差し替え（なおき決定）。旧案「書類に追われる時間を、人と会う時間に戻す」は
     * 「綺麗すぎて損失が見えない」でNG。**この業種で一番の恐怖は候補者の他社決定**なので、
     * そこへ正面から踏み込む。
     *
     * 煽り型に落とさないための線引きが3つある（正本 §1）。**言い回しを変えるときは必ず確認する**:
     * ①読者（会社・担当者）を主語にして責めない ②「奪われる」等の損失を煽る動詞を使わない
     * （`決まっていく` の進行形にとどめる）③数字を添えない。
     *
     * 正本は「21字・1行」と書いているが、**PCの見出しサイズでは1行に収まらない**
     * （21字 × 最大46.4px ≒ 974px > コピー幅820px）。素のbudouxに任せると
     * `候補者は他社で／決まっていく` のように後半が割れる。正本 §1 が
     * 「折り返す場合は読点の位置でのみ折り、`候補者は他社で決まっていく` を単独行にする」と
     * 指定しているので、**読点で2要素に割って折り位置を固定する**（1要素=1ブロックで描画される）。
     * PCはこの2行、SPはさらにbudouxが文節で折る。
     */
    mainCatch: ["返信を待つ間に、", "候補者は他社で決まっていく"],
    /*
  ...
  education: {
    /*
     * 2026-08-11 差し替え。旧「候補者を、待たせない会社になる」は、新しいメインキャッチ
     * 「返信を待つ間に、候補者は他社で決まっていく」の**そのままの裏返し**で、
     * ページで最も大きい2つの見出しが同じことを言う状態になっていた。
     * 待たせることの損失はFVに一本化する（正本 §3）。
     */
